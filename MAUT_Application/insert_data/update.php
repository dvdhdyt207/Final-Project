...
<body>
    ...
    <main class="bg-light">
        <div class="p-2">
            ...
            include 'koneksi.php';
            $np         = $_GET['nip'];
            $gr         = mysqli_query($kon, "select * from tbl_guru where nip='$np'");
            $row        = mysqli_fetch_array($gr);
            ?>
                <div class="container mt-5">
                    <h5 class="mb-4">Update Data Guru</h5>
                    <form method="post" action="updateproses.php">
                        <input type="hidden" value="<?php echo $row['nip']; ?>" name="nip">
                        <div class="mb-3">
                            <label for="nama" class="form-label">NAMA GURU:</label>
                            <input type="text" class="form-control" id="nama" value="<?php echo $row['nama']; ?>" name="nama" required>
                        </div>
                        <div class="mb-3">
                            <label for="nip" class="form-label">NIP</label>
                            <input type="text" class="form-control" id="nip" value="<?php echo $row['nip']; ?>" name="nip" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="mp" class="form-label">MATA PELAJARAN:</label>
                            <input type="text" class="form-control" id="mp" value="<?php echo $row['mp']; ?>" name="mp" required>
                        </div>
                        <div class="mb-3">
